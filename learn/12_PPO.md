...
- 建议先读 `learn/10_Policy_Gradient_REINFORCE.md`，先把“策略直接输出动作概率”这件事吃透。
- 建议先读 `learn/11_Actor_Critic_A2C.md`，先熟悉“演员负责选动作，评论家负责打分”的分工。
- 如果前两篇你还没读，也可以直接看这一篇。本文会把需要的背景在文中补齐。
...
为了让后面的公式和例子不飘在空中，这里先把本章默认沿用的项目背景放在桌面上：

- 当前动作空间是 `192` 个离散动作，也就是 $6 \times 4 \times 4 \times 2$ 个组合。
- GPU 频率常见范围是 `282-710 MHz`，目标帧率盯在 `59-60 FPS`。
- 功耗观测范围可以粗看成 `0-8000 mW`，文中的奖励例子会优先用源文档里反复出现的 `2200-5100 mW` 这段真实量级。
- `RL_DCVS_AI_Integrated_Decision_2026-04-01.md` 引用的样本日志是 `offline_test_iter5/node_dataset_s00_fsd10_pd90_pu85_sf0_20260306-191401-RL.csv`，其中一共 `8095` 行，但 `action_id` 只有一个值 `72`。
- `RL_Algorithm_Selection_for_GPU_DCVS_Tuning_20260401.md` 还给了一个更大的背景：当前总动作覆盖大约只有 `30/192`，离“每种动作都见过很多次”还差得很远。

这组数字很重要，因为 PPO 好不好用，不是只看公式漂不漂亮，还要看它碰到的数据到底长什么样。
...
> 补充知识：同策略（On-Policy）和离策略（Off-Policy）到底差在哪
>
> 你可以把同策略想成“你刚按这套开法跑了几圈，然后马上只根据这几圈来改自己的驾驶习惯”。它最在意的是：训练时看到的数据，必须和当前这版策略足够像。这样好处是逻辑干净，但坏处是旧数据很快就过期。
>
> 离策略更像“除了你自己刚跑的几圈，你还会把别人以前跑过的大量行车记录也拿来学”。这类方法更适合吃固定日志和历史回放，所以在当前这个已经积累了一批逐帧 CSV、但又不适合大规模真机乱试的 DCVS 任务里，CQL、IQL 这类离线强化学习方法会比 PPO 更占便宜。
...
把这件事说得再具体一点，源分析文档里已经给出了一个很典型的样本快照：

| 日志观察项 | 具体数字 | 对 PPO 的直接影响 |
|---|---:|---|
| 样本文件 | `offline_test_iter5/node_dataset_s00_fsd10_pd90_pu85_sf0_20260306-191401-RL.csv` | 这是固定日志，不是边训练边重新采的 on-policy 轨迹 |
| 行数 | 8095 | 行数不算太少，但“条数多”不等于“动作覆盖广” |
| `action_id` 分布 | 只有 `72` | 新策略想比较别的动作时，几乎没有直接参照 |
| 当前总动作覆盖 | 约 `30/192` | 大量动作几乎没见过，策略一旦偏离日志就容易失真 |
