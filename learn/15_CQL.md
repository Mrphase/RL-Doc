...
```mermaid
flowchart TD
    A[输入: 原始逐窗口日志] -->|读取日志| B[处理: 读取当前状态 s_t]
    B -->|提取控制量| C[处理: 读取本窗口动作 a_t]
    C -->|结合下一窗口结果| D[处理: 计算奖励 r_t]
    D -->|对齐时间顺序| E[处理: 取下一窗口状态 s_t+1]
    E -->|拼接字段| F[输出: 生成一条样本 (s_t,a_t,r_t,s_t+1,d_t)]
...
```mermaid
flowchart TD
    A[输入: 当前样本 (s_t,a_t,r_t,s_t+1)] -->|送入目标网络| B[处理: 用目标网络看 s_t+1 的所有动作分数]
    B -->|选最高分| C[处理: 取最大分数 max Q(s_t+1,a')]
    C -->|折回当前时刻| D[处理: 计算目标 y_t]
    A -->|送入当前网络| E[处理: 当前网络输出 Q(s_t,a_t)]
...
```mermaid
flowchart TD
    A[输入: first_step_down 6 档] -->|提供第一维| E[处理: 组合所有参数]
    B[输入: penalty_down 4 档] -->|提供第二维| E
    C[输入: penalty_up 4 档] -->|提供第三维| E
    D[输入: strict_frame 2 档] -->|提供第四维| E
...
```mermaid
flowchart TD
    A[输入: 当前 FPS] -->|按分段规则计算| D[处理: 计算 fps_component]
    B[输入: GPU 频率] -->|代入频率公式| E[处理: 计算 freq_component]
    C[输入: GPU 功耗] -->|代入功耗公式| F[处理: 计算 power_component]
    D -->|送入求和| G[处理: 三项相加]
    E -->|送入求和| G
    F -->|送入求和| G
    G -->|得到总分| H[输出: reward]
