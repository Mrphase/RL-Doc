...
源材料给出的一个关键事实是：当前大约只覆盖了 $30$ 个动作，而总动作数是 $192$ 个。
...
| 关键假设 | 更可能支持的方向 | 背后的理由 |
|---|---|---|
| 动作主要影响当前窗口，未来影响弱 | 上下文赌博机 | 看即时奖励就够用了，在线适应也更直接 |
| 动作会通过热和惯性影响后续窗口 | CQL / IQL 这类离线 RL | 需要看长期回报，而不是只看当前一步 |
| 数据覆盖很差，未见动作很多 | CQL / IQL | 需要对没见过的动作更保守 |
| 真机探索容错很低 | 先离线，后小步上线 | 先用历史日志学，再加安全阀部署 |
...
- 奖励看的是当前这一轮，回报看的是未来很多轮，Q 值就是“当前状态下某个动作到底值不值”的长期评分表。
