...
- 建议先读 `04_Multi_Armed_Bandits.md`。如果你还没读，也可以直接继续，因为本文会先把普通老虎机（Multi-Armed Bandit）快速复习一遍。
...
```mermaid
flowchart TD
    A[输入: 当前上下文 x_t] --> B[处理: 对每个候选动作算 theta]
...
## 4. 再往前一步: Neural Contextual Bandit 和 Neural Thompson Sampling
