...
```mermaid
flowchart TD
    A[输入：第 t 轮开始] -->|从候选里挑一个| B[处理：选择本轮动作编号]
...
```mermaid
flowchart TD
    A[输入：4 个动作编号 的历史奖励序列] -->|先按动作分组| B[处理：更新每个动作的统计量]
...
flowchart TD
    ...
    B -->|形成上下文 上下文向量| C[处理：过滤明显危险的动作]
    C -->|只在安全候选里比较| D[处理：根据上下文给动作打分]
    D -->|输出本轮选择| E[输出：得到新的动作编号]
