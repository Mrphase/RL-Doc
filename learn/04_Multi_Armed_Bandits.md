...
- 建议先读 `learn/02_RL_Core_Concepts.md`，先熟悉奖励、策略、在线决策这些最基础词汇。
- 建议再读 `learn/03_Exploration_vs_Exploitation.md`，先把“探索”和“利用”为什么会打架想明白。
- 如果你前面还没读全，也没关系。本篇会尽量自包含。默认你已经知道：GPU 频率常见范围是 `282-710 MHz`，目标 FPS 是 `59-60`，功耗观测范围常按 `0-8000 mW` 理解，动作空间是 `6×4×4×2 = 192` 个离散动作。
...
再把 `30/192 = 15.6%` 这件事翻成工程时间，你会更明白为什么第一、二份文档更保守。

| 指标 | 数值 | 含义 |
|---|---:|---|
| 总动作数 | `192` | `6×4×4×2` 个离散组合 |
| 已覆盖动作数 | `30` | 离线日志里真正见过的动作 |
| 未覆盖动作数 | `162` | 线上如果想摸清，只能继续试 |
| 目标最少试验次数 | `5 次/动作` | 这还只是为了拿到很粗的平均分 |
...
这表示：只为了让 `162` 个没见过的动作各试 `5` 次，就已经需要 `810` 个探索窗口。

如果为了安全，你把探索比例压到 `10%`，也就是每 `10` 个窗口只有 `1` 个窗口敢拿来试新动作，那么总决策窗口数至少是：
...
$$
8100 \times 5 = 40500 \text{ 秒}
$$

$$
40500 \div 60 = 675 \text{ 分钟}
$$

$$
675 \div 60 = 11.25 \text{ 小时}
$$

这个 `11.25` 小时还只是“每个未覆盖动作先摸 5 次”的很粗下限，甚至还没算下面这些现实成本：

- 不同负载上下文下，同一个动作还得重复试；
- 热漂移和 governor 惯性会让早上试出来的均值，下午不一定还准；
- 某些动作可能一试就掉到 `59 FPS` 以下，根本不适合继续在线探索。

所以三份源文档看起来像是在争论“到底该不该上 bandit”，本质上其实是在争论另一件更工程化的事：

> **我们到底有没有资格，在真机上用纯在线的办法，把这 192 个动作慢慢试明白？**

如果答案偏“没有”，那 CQL、IQL 和安全壳就会靠前；如果答案偏“有，但必须很小心”，那安全上下文老虎机就会靠前。
