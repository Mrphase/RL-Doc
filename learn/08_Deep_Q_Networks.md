...
- 建议先读 `learn/02_RL_Core_Concepts.md`
- 建议先读 `learn/03_Exploration_vs_Exploitation.md`
- 建议先读 `learn/07_Tabular_Q_Learning.md`
- 如果前面几篇你还没完全吃透，这一篇也可以独立阅读，但最好先知道状态、动作、奖励、Q 值分别是什么意思。
...
> **补充知识：归一化（Normalization）到底在做什么**
>
> 归一化可以先理解成“把不同单位的量，压到差不多的数值尺度上”。不然的话，GPU 频率动不动就是几百 MHz，功耗又是几千 mW，FPS 却只有 50 多到 60，网络会更容易被大数字牵着走。
>
> 最常见的做法之一，就是把一个量映射到 0 到 1 之间。比如 GPU 频率范围是 282 到 710 MHz，那 430 MHz 的归一化值就不是直接拿 430，而是看它在这个范围里处在什么位置。
...
现在用一组真实量级的 DCVS 数字，把“原始状态怎么变成网络输入”手算一次。
...
$$
x = [0.9733,\ 0.0267,\ 0.86,\ 0.3458,\ 0.4,\ 0.3770,\ \dots]
$$

接下来，网络会把这 19 维输入一路映射成 192 个动作分数。假设输出里几个动作的分数是：

$$
Q(s,72)=20.4,\quad Q(s,88)=21.1,\quad Q(s,103)=19.8
$$

那么当前网络就会先倾向于把 88 号动作看成更优候选。你可以把这一步理解成：**状态设计负责把现场描述清楚，DQN 负责把这个现场翻译成 192 个动作的评分表。**
...
如果 mini-batch 大小是 $m$，那每次训练拿到的一批样本可以写成：

$$
\mathcal{B} = \{(s_j, a_j, r_j, s'_j)\}_{j=1}^{m}, \quad \mathcal{B} \subset \mathcal{D}
$$

随机采样的目的不是“变花哨”，而是为了打散时间相关性。连续几帧常常高度相似，如果直接拿来连着学，网络会来回追着最近一小段波动跑。

这张图展示了经验回放是怎么把“按时间排好的经历”打散成“训练时随机抽样的批次”的。

```mermaid
flowchart TD
    A[按时间产生转移样本] --> B[写入经验回放缓冲区]
    B --> C[缓冲区里混有高负载和低负载样本]
    C --> D[训练时随机抽取 mini-batch]
    D --> E[一批样本不再只来自最近几帧]
    E --> F[更新更稳，不容易追着短期波动跑]
...
图里的关键路径是：**先把不同场景的历史都装进缓冲区，再在训练时故意打乱顺序抽样，这样网络看到的就不只是“眼前这几帧”。**
...
现在假设 mini-batch 大小是 2，而且不放回随机抽样。所有可能抽到的两条组合一共只有 6 种：
...
如果我们关心“低频但高风险的 D 会不会被抽到”，就一步一步算：

1. 一共 6 种可能组合。
2. 里面包含 D 的有 3 种：A + D、B + D、C + D。
3. 所以 D 被抽中的概率是：
...
也就是说，就算 D 不是最近一帧，它仍有 50% 的概率进入这次训练。

再看一次具体批次。如果只盯着最近两个“看上去不错”的样本 A 和 B，那么平均奖励是：
...
你会发现，随机抽样把“坏场景”也重新带回训练里了。这样网络不会只因为最近几次高帧率就误以为所有动作都很安全。
...
这张图展示了目标网络为什么能把训练靶子先固定住。

```mermaid
flowchart TD
    A[当前状态输入当前Q网络] --> B[当前网络输出 Q(s,a;θ)]
    C[下一状态输入目标网络] --> D[目标网络输出 max Q(s',a';θ^-)]
    B --> E[计算损失并更新当前网络]
    D --> F[组成固定的 Bellman 目标]
    F --> E
    E --> G[每隔若干步复制 θ 到 θ^-]
...
图里的关键路径是：**当前网络负责学习，目标网络负责暂时当固定参考答案。等当前网络学一阵子后，再整体同步一次。**
...
这张图展示了 $\varepsilon$-贪心是怎么在“利用已有经验”和“故意试新动作”之间分流的。

```mermaid
flowchart TD
    A[输入当前状态] --> B{抛一次 ε 概率硬币}
    B -->|90%| C[走贪心分支 选当前最高分动作]
    B -->|10%| D[走探索分支 在192个动作里随机抽]
    C --> E[执行动作并观察 FPS 与功耗]
    D --> E
    E --> F[把结果写回经验池继续学习]
...
图里的关键路径是：**大部分时间用当前最优动作保住表现，小部分时间留下探索通道，防止策略永远卡在旧经验里。**
